...
contract SupplyChain {
  ...
  modifier forSale(uint sku) {
    ...
    require(items[sku].seller != address(uint160(0)), "item does not exist");
    _;
  }
  modifier sold(uint _sku) {
    require(items[_sku].state == State.Sold, "item is not in the sold state");
    _;
  }
  modifier shipped(uint _sku) {
    require(items[_sku].state == State.Shipped, "item is not in the shipped state");
    _;
  }
  modifier received(uint _sku) {
    require(items[_sku].state == State.Received, "item is not in the received state");
    _;
  }
  ...
  function addItem(string memory _name, uint _price) public returns (bool) {
    ...
    address payable buyer = address(uint160(0));
    items[skuCount] = Item({
     name: _name, 
     sku: skuCount, 
     price: _price, 
